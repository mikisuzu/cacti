...
double Mat::compute_bit_mux_sa_precharge_sa_mux_wr_drv_wr_mux_h()
{
 ...
 // double precharge_h = compute_tr_width_after_folding(g_tp.w_pmos_bl_precharge, cell.w / (2 *(RWP + ERP))) +
 ...
 //  double col_mux_tr_height = compute_tr_width_after_folding(g_tp.w_nmos_b_mux, cell.w / (2 *(RWP + ERP)));
 ...
 // double sense_amp_h = height_sense_amplifier(cell.w * deg_bl_muxing / (RWP + ERP));
  ...
  if (dp.Ndsam_lev_2 > 1)
  {
   ...
   // double sense_amp_mux_height = compute_tr_width_after_folding(
          //  g_tp.w_nmos_sa_mux, cell.w * deg_bl_muxing * dp.Ndsam_lev_1 / (RWP + ERP));
    //height_senseamp_mux_decode_output_wires =  Ndsam * wire_inside_mat_pitch * (RWP + ERP);
  ...
  //  double sense_amp_mux_h_dram = 2 * compute_tr_width_after_folding(
         //   pmos_to_nmos_sz_ratio(is_dram) * g_tp.min_w_nmos_, cell.w * dp.Ndsam_lev_2 / (RWP + ERP));
    height += 2 * compute_tr_width_after_folding(g_tp.min_w_nmos_, cell.w * dp.Ndsam_lev_2 / (RWP + ERP));
   // double sense_amp_mux_h = 2 * compute_tr_width_after_folding(g_tp.min_w_nmos_, cell.w * dp.Ndsam_lev_2 / (RWP + ERP));
  }
...
double Mat::compute_bitline_delay(double inrisetime)
{
 ...
 // double leak_power_nmos_l_dram_cell = 0;
 // double leak_power_nmos_ground_dram_cell = 0;
 // double dram_V_swing = 0.11;
  ...
  if (is_dram == true)
  {
   ...
   // leak_power_nmos_l_dram_cell = simplified_nmos_leakage(g_tp.dram.cell_nmos_w, is_dram, false, true);
   // leak_power_nmos_ground_dram_cell = simplified_nmos_leakage(g_tp.dram.cell_nmos_w, is_dram, false, true) * 1.2;
   // power_bitline.readOp.leakage = leak_power_nmos_l_dram_cell + leak_power_nmos_ground_dram_cell;
  }
  else
  { //SRAM
  ...
  if (is_dram) 
  {
   ...
   // dynRdEnergy += (C_bl + 2*C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) *
    //  dram_V_swing * g_tp.dram_cell_Vdd * subarray.num_cols * num_subarrays_per_mat;
    dynRdEnergy += (C_bl + 2*C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) *
...
void Mat::compute_power_energy()
{
  ...
  if (is_fa == false || dp.is_tag == false)
  {
    ...
    power_output_driver = num_do_b_mat;
